...
## Step 1 - finding coaching text

- Find shooting coaching videos, write simple script to extract audio with yt-dlp
Code:
``` sh
#!/bin/bash
yt-dlp -a "../data/yt_links.txt" -x --audio-format mp3 -o "../data/audio/audio_%(autonumber)s.mp3"
```

## Step 2 - transcribe audio to text

- Parse each audio file, extract important sequences and transcribe it to text with whisper
Code:
``` python
# transcribe.py
...
## Step 3 - ingest data, attach metadata fields to it

- Parse each `.JSON` file previously created, extract the embeddings and attach metadata fields to it to store in `ChromaDB`
Code:
``` python
# ingest.py
```
